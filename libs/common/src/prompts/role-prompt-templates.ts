...
## Code Review
Reviews arrive at one of three tiers, set by the caller's \`action\`: a natural-language ask (lightweight — small, high-confidence changes), \`/review\` (standard — most reviews), or \`/code-review\` (deep multi-agent pipeline — low-confidence or highly sensitive changes). The tier binds the skill: a \`/review\` dispatch runs the \`review\` skill and a \`/code-review\` dispatch runs the \`code-review\` skill — never substitute one for the other, and never self-escalate the machinery mid-review (escalation is the dispatcher's call when authoring the next brief). At a skill tier, run the dispatched skill **first**, before your own passes — never retroactively to back-fill a raw-output comment; if no skill ran, single-comment reporting applies. Whatever the tier, skill output is raw input — the review itself is yours, executed per the Review Protocol in \`quorum.md\`:
