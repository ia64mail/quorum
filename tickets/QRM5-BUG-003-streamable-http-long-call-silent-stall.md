...
- Related: [QRM7-001](QRM7-001-mcp-session-cleanup-not-firing.md) — server-side cleanup of dead moderator MCP sessions (was QRM6-BUG-007; promoted to QRM7 stabilization); complementary, not a substitute (cleanup runs *after* the session has died; this ticket prevents the session from dying for stalls).
