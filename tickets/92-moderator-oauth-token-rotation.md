...
- [x] Ruled-out theory (CC CLI version bump moving/locking the credential path) is recorded, along with the corrected finding (still pinned at 2.1.126, no such ticket/bump exists)
...
- `Dockerfile:128` — CC CLI pinned at `@anthropic-ai/claude-code@2.1.126` (QRM6-001 spike pin; no later bump found in git history — the previously assumed "#68 upgrade to 2.1.207" does not exist in this repo and should not be cited as a cause).
