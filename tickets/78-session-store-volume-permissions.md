...
- [x] The three `*-sessions` named volumes (`quorum_architect-sessions`, `quorum_teamlead-sessions`, `quorum_developer-sessions`) are re-initialized under `quorum:quorum` ownership via the drop-and-recreate procedure documented above; `docker exec <agent> ls -ldn /var/agent-sessions` reports the correct uid/gid on each.
- [x] `npm run build`, `npm run lint`, `npm run test` all green on the amended branch.
- [x] Phase-3 durability gate executed through the moderator: codeword planted, `--force-recreate developer`, codeword recalled on resume; `/var/agent-sessions/<sessionId>.jsonl` non-empty; no `mirror_error` warn line in the invocation window. Results recorded in Implementation Notes.
- [x] #68 AC-8 (Verification Runbook Checks 0–13) re-executed post-fix; Check 12 flipped to `[x]` in `tickets/68-bump-agent-sdk-cc-cli-opus-4-8.md` with a cross-reference to this ticket for the underlying repair.
...
### Phase-3 durability verification (2026-07-16, operator + moderator)

The end-to-end durability gate specified in Implementation Details §5 was executed jointly by the operator (host-side volume repair + `--force-recreate`) and the moderator (codeword plant + post-recreate recall) on 2026-07-16. **Result: PASS.** All expected observations held; no regression signals surfaced.

**Ownership flip (operator step):** After the drop-and-recreate procedure (`docker compose stop architect teamlead developer` → `docker volume rm quorum_{architect,teamlead,developer}-sessions` → `./scripts/start.sh -d`), `docker compose exec developer ls -ld /var/agent-sessions` reports `drwxr-xr-x quorum quorum` (previously `root:root`). Docker re-initialized the mountpoint from the rebuilt image path with the new `chown -R quorum:quorum` layer, so `FileSessionStore.append()` can now write without `EACCES`.

**Pre-recreate mirror write (moderator step 1):** Codeword `SALAMANDER-78-EACCES-4471` planted in developer session `e6a881b9-95c4-4ef1-b4d2-a29ddd08a4b4` through the moderator. `docker compose exec developer ls -la /var/agent-sessions/` shows `/var/agent-sessions/e6a881b9-95c4-4ef1-b4d2-a29ddd08a4b4.jsonl` present at **59891 bytes** — a substantive non-zero mirror. Two additional session mirror files from other invocations were also present in the same directory, confirming that `FileSessionStore.append()` is now writing durably across multiple invocations, not just the codeword session. No `mirror_error` warn line in the invocation window (grep of `logs/developer-*.jsonl` returned no matches for the invocation).

**Force-recreate (operator step):** `docker compose up -d --force-recreate developer` produced a fresh container **`1ba3cbcf1786`** — new container id confirming the process truly restarted from a clean image state. The named volume `quorum_developer-sessions` persisted through the recreate and re-mounted into the fresh container with the same `quorum:quorum` ownership and contents intact.

**Post-recreate recall (moderator step 2):** Follow-up dispatch to the developer on the fresh container `1ba3cbcf1786`, resuming `sessionId=e6a881b9-95c4-4ef1-b4d2-a29ddd08a4b4`, recalled the codeword verbatim — `SALAMANDER-78-EACCES-4471` — with no `No conversation found with session ID` error and no `Session resume failed … — retrying fresh` log line. This confirms both `FileSessionStore.append()` (pre-recreate durability write) **and** `FileSessionStore.load()` (post-recreate read-back on the fresh container) are functioning end-to-end on SDK 0.3.207 with the volume now `quorum`-owned.

**Property closed:** QRM8 D3's cross-container-restart session-resume guarantee (#10) is now empirically verified on the current codebase. The `EACCES`-on-named-volume failure class recorded in PR #69 Finding 3 is eliminated, and the `mirror_error` observability branch added in this ticket did not fire during the healthy invocation — the belt-and-braces observability is quiet on a good volume, as intended. #68 AC-8 Check 12 (session resume durability across restart) inherits this pass; the cross-reference note now lives on #68's AC-8 line.
