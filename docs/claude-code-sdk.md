...
| `ANTHROPIC_MODEL` | `claude-opus-4-8` | Model for SDK queries (fleet-wide default via `x-shared-env`) |
| `<ROLE>_ANTHROPIC_MODEL` | — (unset) | Per-role override for `ANTHROPIC_MODEL`, wired on each deployed agent service (`ARCHITECT_`, `TEAMLEAD_`, `DEVELOPER_`) in `docker-compose.yml` after the `<<: *shared-env` merge. Resolution order: `<ROLE>_ANTHROPIC_MODEL` → `ANTHROPIC_MODEL` → committed default. See ticket #80. |
