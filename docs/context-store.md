...
2. **Caller-side enrichment** — two callers wrap the backend trace with session/correlation metadata and a `source` tag before emitting:
   ...
   - `BootstrapContextService.selectProjectItems()` (invoked from `assemble()`, which threads `correlationId`) generates its own `queryId`, tags `source: 'bootstrap'`, and sets `callerRole: null` (no invoking agent). It emits when the ranked search produces the returned selection *or* when it throws after `onTrace` already captured a trace (mirroring the error-path semantics above); the recency-fallback branch — triggered by an absent query, an InMemory backend, a search that completes with zero hits, or a search that throws before capturing any trace — emits nothing, so an absent bootstrap trace reads as "recency fallback ran," not "observability broken."
